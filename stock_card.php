...
<form class="stock-card" method="get">
    ...
    <div class="actions-row"><div></div><div class="stock-actions"><button class="btn accent" type="submit">Afișează</button><a class="btn" target="_blank" href="stock_card_export_pdf.php?date_from=<?= urlencode($dateFrom) ?>&date_to=<?= urlencode($dateTo) ?>&group=<?= urlencode($group) ?>&product_id=<?= (int)$productId ?>">Export PDF</a></div></div>
</form>
...
<div class="stock-card"><h2 style="margin:0 0 14px;font-size:18px;">Fișa magazie - <?= stock_h($dateFrom) ?> - <?= stock_h($dateTo) ?></h2><div class="stock-table-wrap"><table class="stock-table"><thead><tr><th>Produs</th><th>Grupă</th><th>UM</th><th>Stoc inițial</th><th>Intrări</th><th>Consum / ieșiri</th><th>Stoc final</th><th>Status</th></tr></thead><tbody>
