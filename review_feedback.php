...
<head>
<meta charset="UTF-8">
<title>Feedback clienți</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, viewport-fit=cover">
<?php app_theme_css(); ?>
<style>
.page{max-width:1320px;margin:0 auto;display:flex;flex-direction:column;gap:14px}.hero{background:var(--surface);border:1px solid var(--border);border-radius:8px;box-shadow:none;padding:18px 20px;display:flex;justify-content:space-between;gap:14px;align-items:center;flex-wrap:wrap}.hero h1{margin:0;font-size:24px;letter-spacing:0}.hero p{margin:5px 0 0;color:var(--muted);font-size:12.5px}.btn{border:0;border-radius:4px;background:var(--accent);color:#fff;font-weight:800;padding:8px 12px;min-height:34px;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;font-size:12.5px}.btn.ghost{background:#fff;color:var(--text);border:1px solid var(--border)}.cards{display:grid;grid-template-columns:repeat(6,1fr);gap:10px}.stat{background:var(--surface);border:1px solid var(--border);border-radius:8px;box-shadow:none;padding:12px}.stat span{display:block;color:var(--muted);font-size:10.5px;text-transform:uppercase;letter-spacing:0;font-weight:800}.stat strong{display:block;font-size:22px;letter-spacing:0;margin-top:3px}.panel{background:var(--surface);border:1px solid var(--border);border-radius:8px;box-shadow:none;overflow:hidden}.filters{padding:12px;border-bottom:1px solid var(--border);display:flex;gap:8px;flex-wrap:wrap;align-items:center}.filters select,.filters input{border:1px solid var(--border);border-radius:4px;padding:7px 10px;min-height:34px;font:inherit;font-size:12.5px;background:#fff}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:980px}th,td{padding:10px 12px;border-bottom:1px solid var(--border);text-align:left;vertical-align:top}th{font-size:10.5px;color:var(--muted);text-transform:uppercase;letter-spacing:0;background:#f8fafc}td{font-size:12.5px}.client{font-weight:800}.muted{color:var(--muted);font-size:12px}.badge{display:inline-flex;align-items:center;border-radius:4px;padding:4px 7px;font-size:11.5px;font-weight:800;border:1px solid var(--border);background:#fff}.badge.good{background:#ecfdf5;color:#047857;border-color:#bbf7d0}.badge.warn{background:#fffbeb;color:#b45309;border-color:#fde68a}.badge.bad{background:#fef2f2;color:#dc2626;border-color:#fecaca}.notice{border-radius:8px;padding:10px 12px;font-weight:650;background:#EEF8FF;color:#002050;border:1px solid #B1D6F0;font-size:12.5px}.answers{font-size:12px;color:var(--muted);line-height:1.5}.actions{display:flex;gap:8px;flex-wrap:wrap}@media(max-width:900px){.cards{grid-template-columns:repeat(3,1fr)}.hero{align-items:flex-start}.page{max-width:100%}}@media(max-width:600px){.page{gap:10px}.hero{padding:14px}.hero h1{font-size:21px}.hero p{display:none}.topbar strong{display:none}.actions{display:grid;grid-template-columns:1fr 1fr;width:100%}.actions form,.actions .btn{width:100%}.actions form:first-child{grid-column:1/-1}.cards{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.stat{padding:10px;min-height:76px}.stat strong{font-size:22px}.filters{display:grid;grid-template-columns:1fr 1fr;gap:8px}.filters select{grid-column:1/-1}.filters .btn{width:100%}table{min-width:720px}th,td{padding:9px 10px}}
</style>
</head>
<body>
<div class="layout">
        ...
        <div class="content page">
            ...
            <section class="panel">
                <form class="filters" method="get">
                    ...
                    <a class="btn ghost" href="review_feedback.php">Resetează</a>
                </form>
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            <?php if (!$rows): ?>
                                <tr><td colspan="9" class="muted">Nu există solicitări de review.</td></tr>
                            <?php endif; ?>
                            <?php foreach ($rows as $row): ?>
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge <?= $channel === 'email' ? 'warn' : 'good' ?>"><?= pz_review_h(strtoupper($channel)) ?></span><br><span class="muted"><?= $isFirst ? 'prima intervenție' : 'intervenție ulterioara' ?></span><br><span class="muted">Google: <?= $allowGoogle ? 'permis' : 'blocat' ?></span></td>
