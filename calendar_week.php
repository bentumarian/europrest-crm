...
function cw_ro_label(string $date): string {
    $d = new DateTime($date);
...
for ($i = 0; $i < 7; $i++) {
    ...
    $weekDates[] = ['date' => $d->format('Y-m-d'), 'label' => cw_ro_label($d->format('Y-m-d'))];
}
...
if (!$teams) {
    $teams = [['id' => 0, 'name' => 'Tehnician', 'color' => '#163B63']];
}
$teamIndexById = [];
...
try {
    ...
    if (cw_table_exists($pdo, 'appointment_teams')) {
        ...
        foreach (array_merge($appointments, $extra) as $a) {
            $key = (int)$a['id'] . '-' . (int)$a['display_team_member_id'];
            $seen[$key] = $a;
        }
        $appointments = array_values($seen);
    }
} catch (Throwable $e) {
    error_log('Calendar week error: ' . $e->getMessage());
}

$appointmentsByGrid = [];
foreach ($appointments as $a) {
    $teamId = (int)($a['display_team_member_id'] ?? 0);
    if (!isset($teamIndexById[$teamId])) continue;
    $appointmentsByGrid[] = $a;
}

$pz_page_title = 'Calendar';
$pz_page_breadcrumbs = ['Saptamana pe tehnicieni'];
?><!doctype html>
<html lang="ro">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Calendar saptamanal - CRM</title>
    <?php app_theme_css(); ?>
    <style>
    .cw-toolbar{display:flex;gap:10px;align-items:center;justify-content:space-between;margin:0 0 14px;flex-wrap:wrap}.cw-left,.cw-right{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.cw-date{min-width:150px;text-align:center}.cw-select{min-width:210px}.cw-card{background:#fff;border:1px solid var(--border);border-radius:18px;box-shadow:var(--shadow-md);overflow:hidden}.cw-scroll{overflow:auto;max-height:calc(100vh - 150px);-webkit-overflow-scrolling:touch}.cw-grid{display:grid;grid-template-columns:60px repeat(var(--cw-cols),minmax(42px,1fr));grid-template-rows:34px 38px repeat(36,26px);min-width:max(100%,calc(60px + var(--cw-cols) * 42px));position:relative}.cw-corner{position:sticky;left:0;top:0;z-index:50;background:#fff;border-right:1px solid #CBD5E1;border-bottom:1px solid #CBD5E1}.cw-day{position:sticky;top:0;z-index:35;display:flex;align-items:center;justify-content:center;background:#F8FAFC;border-left:1px solid #E2E8F0;border-bottom:1px solid #CBD5E1;font-size:11px;font-weight:900;color:#002050;letter-spacing:.04em}.cw-team{position:sticky;top:34px;z-index:34;display:flex;align-items:center;justify-content:center;background:color-mix(in srgb,var(--team-color) 18%,#fff);border-left:1px solid #E2E8F0;border-bottom:1px solid #CBD5E1;box-shadow:inset 0 3px 0 var(--team-color)}.cw-dot{width:24px;height:24px;border-radius:999px;background:var(--team-color);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:10px}.cw-time{position:sticky;left:0;z-index:20;background:#fff;border-right:1px solid #CBD5E1;border-bottom:1px solid #E2E8F0;display:flex;align-items:center;justify-content:flex-end;padding-right:8px;font-family:var(--mono);font-size:10px;font-weight:700;color:#002050}.cw-slot{background:color-mix(in srgb,var(--team-color) 5%,#fff);border-left:1px solid #EEF2F7;border-bottom:1px solid #EEF2F7;cursor:pointer}.cw-slot:hover{background:color-mix(in srgb,var(--team-color) 16%,#fff);outline:2px solid color-mix(in srgb,var(--team-color) 55%,transparent);outline-offset:-2px}.cw-hour{border-top:1px solid #CBD5E1}.cw-event{z-index:25;margin:3px;border-radius:8px;border:1px solid rgba(0,32,80,.18);box-shadow:0 8px 18px rgba(0,32,80,.13);cursor:pointer;position:relative;overflow:hidden}.cw-event:after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.28),transparent 56%)}.cw-event.done:before{content:'✓';position:absolute;right:4px;top:3px;z-index:2;background:rgba(255,255,255,.88);color:#002050;border-radius:99px;width:14px;height:14px;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:900}.cw-modal-backdrop{display:none;position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:400;padding:20px;overflow:auto}.cw-modal-backdrop.open{display:block}.cw-modal{background:#fff;border-radius:18px;max-width:680px;margin:30px auto;padding:18px;border:1px solid var(--border);box-shadow:var(--shadow-lg)}.cw-modal-head{display:flex;justify-content:space-between;gap:12px;align-items:center;border-bottom:1px solid var(--border);padding-bottom:12px;margin-bottom:14px}.cw-form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.cw-full{grid-column:1/-1}.cw-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:14px}@media(max-width:860px){.cw-grid{grid-template-columns:46px repeat(var(--cw-cols),minmax(36px,1fr));grid-template-rows:32px 36px repeat(36,24px);min-width:calc(46px + var(--cw-cols) * 36px)}.cw-time{font-size:8px;padding-right:4px}.cw-day{font-size:9px}.cw-dot{width:21px;height:21px;font-size:9px}.cw-form-grid{grid-template-columns:1fr}}
    </style>
</head>
<body>
<div class="layout">
    ...
    <main class="main">
        <div class="content">
            <div class="cw-toolbar">
                <div class="cw-left">
                    <a class="btn" href="calendar.php?date=<?= cw_h($prevWeek) ?>&view=week&team=<?= cw_h($selectedTeam) ?>">‹</a>
                    <a class="btn orange" href="calendar.php?date=<?= date('Y-m-d') ?>&view=week&team=<?= cw_h($selectedTeam) ?>">Azi</a>
                    <a class="btn" href="calendar.php?date=<?= cw_h($nextWeek) ?>&view=week&team=<?= cw_h($selectedTeam) ?>">›</a>
                    <span class="btn cw-date"><?= cw_h($weekStartObj->format('d.m.Y')) ?> - <?= cw_h($weekEndObj->format('d.m.Y')) ?></span>
                </div>
                <div class="cw-right">
                    <a class="btn" href="calendar.php?date=<?= cw_h($date) ?>&view=day&team=<?= cw_h($selectedTeam) ?>">Zi</a>
                    <a class="btn accent" href="calendar.php?date=<?= cw_h($date) ?>&view=week&team=<?= cw_h($selectedTeam) ?>">Saptamana</a>
                    <a class="btn" href="calendar.php?date=<?= cw_h($date) ?>&view=month&team=<?= cw_h($selectedTeam) ?>">Luna</a>
                    <form method="get" action="calendar.php" style="display:flex;gap:8px;align-items:center;">
            ...
            <div class="cw-card"><div class="cw-scroll">
                ...
                <div class="cw-grid" style="--cw-cols:<?= (int)$cols ?>;">
                    ...
                    <?php foreach ($weekDates as $dIndex => $d): $startCol = 2 + $dIndex * count($teams); ?>
                        ...
                        <?php foreach ($teams as $tIndex => $team): $color = cw_clean_color($team['color'] ?? null); $col = $startCol + $tIndex; ?>
                            <div class="cw-team" style="grid-column:<?= $col ?>;grid-row:2;--team-color:<?= cw_h($color) ?>;" title="<?= cw_h($team['name']) ?>"><span class="cw-dot"><?= cw_h(cw_initials($team['name'])) ?></span></div>
                        <?php endforeach; ?>
                    <?php endforeach; ?>
                    ...
                    <?php foreach ($slots as $slotIndex => $slot): $row = $slotIndex + 3; $isHour = substr($slot, -3) === ':00'; ?>
                        ...
                        <?php foreach ($weekDates as $dIndex => $d): $startCol = 2 + $dIndex * count($teams); ?>
                            <?php foreach ($teams as $tIndex => $team): $color = cw_clean_color($team['color'] ?? null); $col = $startCol + $tIndex; ?>
                                <div class="cw-slot <?= $isHour ? 'cw-hour' : '' ?>" style="grid-column:<?= $col ?>;grid-row:<?= $row ?>;--team-color:<?= cw_h($color) ?>;" onclick="cwOpenCreate('<?= cw_h($d['date']) ?>','<?= cw_h($slot) ?>','<?= (int)$team['id'] ?>')"></div>
                            <?php endforeach; ?>
                        <?php endforeach; ?>
                    <?php endforeach; ?>
                    ...
                    <?php foreach ($appointmentsByGrid as $a):
                        ...
                        $baseColor = cw_clean_color($a['display_team_color'] ?? null);
...
<div class="cw-modal-backdrop" id="cwCreateModal">
    <div class="cw-modal">
        ...
        <form method="post" action="calendar.php" id="cwCreateForm">
            ...
            <input type="hidden" name="redirect_team" value="<?= cw_h($selectedTeam) ?>">
            ...
            <div class="cw-form-grid">
                <div class="cw-full"><label>Client *</label><select name="client_id" required><option value="">Alege clientul</option><?php foreach ($clients as $c): ?><option value="<?= (int)$c['id'] ?>" data-address="<?= cw_h($c['address'] ?? '') ?>" data-phone="<?= cw_h($c['phone'] ?? '') ?>"><?= cw_h($c['name']) ?></option><?php endforeach; ?></select></div>
